...
module.exports = function(options){
  ...
  if(isDir(dest)){
    writeToDir(dest, files, options.wrapper, options.path)
  }else{
...
function writeToDir(dest, files, wrapper, basePath){
  mkdirs(dest);

  files.forEach(function(file){
    var filePath = path.relative(basePath, file);
    var destFile = path.resolve(dest, path.dirname(filePath), path.basename(filePath, path.extname(filePath))) + '.js';
    writeToSingleFile(destFile, [file], wrapper);
  });
}
